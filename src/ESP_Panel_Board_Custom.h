...
#define ESP_PANEL_LCD_RGB_BOUNCE_BUF_SIZE \
    (0) // Bounce buffer size in bytes. This function is used to avoid screen drift.
...
#define ESP_PANEL_LCD_FLAGS_AUTO_DEL_PANEL_IO \
    (0) // Delete the panel IO instance automatically if set to 1.
...
#define ESP_PANEL_TOUCH_I2C_ADDRESS \
    (0) // Typically set to 0 to use the default address.
...
/* Reset pin */
#define ESP_PANEL_TOUCH_IO_RST \
    (-1)                              // IO num of RESET pin, set to -1 if not use
...
/* Interrupt pin */
#define ESP_PANEL_TOUCH_IO_INT \
    (16)                              // IO num of INT pin, set to -1 if not use
...
#define ESP_PANEL_EXPANDER_I2C_ADDRESS \
    (0x20) // The actual I2C address. Even for the same model of IC,
